...
class GtalkClient {
  ...
  /** Bắn 1 tin ẢNH (có caption) vào channelId. */
  async sendPhoto(channelId, { fileId, width, height, caption = '' }) {
    ...
    const r = await postJSON(this.base, '/api/gtalk/send-message', {
      ...
      content: { attachment: { caption, items: [{ image: { fileId: String(fileId), width, height } }] } },
      oaToken: this.oaToken,
    });
  ...
  /** Gửi ẢNH (đã có buffer+dims) cho 1 nhân viên; caption = text. */
  async sendImageToEmployee(employeeId, img, caption = '') {
    const channelId = await this.createDirectChannel(employeeId);
    const fileId = await this.uploadImage(channelId, img);
    const globalMsgId = await this.sendPhoto(channelId, { fileId, width: img.width, height: img.height, caption });
