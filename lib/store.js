...
function supaReq(method, pathQuery, body, extraHeaders = {}) {
  return new Promise((resolve, reject) => {
    ...
    }, (res) => { const ch = []; res.on('data', (c) => ch.push(c)); res.on('end', () => { const d = Buffer.concat(ch).toString('utf8'); let j = null; try { j = d ? JSON.parse(d) : null; } catch { /* */ } resolve({ status: res.statusCode, json: j, raw: d }); }); });
    req.on('error', reject);
...
async function getConfig() {
  ...
  if (process.env.OA_TOKEN) {
    return {
      ...
      // Data API Query (Trino) — danh bạ tổ chức. Không có token thì tab Sơ đồ tổ chức chỉ đọc cache.
      dataApiToken: env('DATA_API_TOKEN'),
      dataApiHost: env('DATA_API_HOST'),
    };
  }
  const cfg = fileRead('config.json', null);
  // Local: cho phép để token trong env (gtalkbot.env) thay vì config.json
  if (cfg && !cfg.dataApiToken && env('DATA_API_TOKEN')) {
    cfg.dataApiToken = env('DATA_API_TOKEN');
    cfg.dataApiHost = env('DATA_API_HOST');
  }
  return cfg;
}

// Lịch crawl chung mặc định (tắt sẵn — người dùng bật khi cấu hình xong). Giờ VN.
// Tự-gửi không nằm ở đây: mỗi lộ trình tự gửi theo campaign.sendTime khi đang bật.
// newHiresEnabled/newHiresTime: lịch riêng cho "NV mới mỗi ngày" (Data API), tách khỏi crawl MCP.
const DEFAULT_SCHEDULE = { crawlEnabled: false, crawlTime: '07:00', newHiresEnabled: false, newHiresTime: '08:00' };
...
module.exports = {
  ...
  // Nhật ký hoạt động theo ngày (Bot đã gửi cho ai/nội dung gì). Shape: { 'YYYY-MM-DD': [entry,...] }
  getActivity: () => getKV('activity', 'activity-log.json', {}),
  setActivity: (a) => setKV('activity', 'activity-log.json', a),
  getSchedule: () => getKV('schedule', 'schedule.json', DEFAULT_SCHEDULE),
  setSchedule: (s) => setKV('schedule', 'schedule.json', s),
  // Danh bạ tổ chức kéo từ Data API (dim_employee). Lưu dạng cột { columns, rows } cho gọn
  // — xem lib/orgdir.js. Cỡ ~1.8MB với 22.6k NV đang làm.
  getOrgDir: () => getKV('org-dir', 'org-dir.json', null),
  setOrgDir: (d) => setKV('org-dir', 'org-dir.json', d),
  // NV mới theo ngày chạy: { 'YYYY-MM-DD': { runAt, days:[...], items:[...] } }
  getNewHires: () => getKV('new-hires', 'new-hires.json', {}),
  setNewHires: (d) => setKV('new-hires', 'new-hires.json', d),
};
