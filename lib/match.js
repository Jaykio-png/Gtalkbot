...
const VN_OFFSET_MS = 7 * 3600 * 1000; // toàn hệ thống tính theo giờ VN (UTC+7)
...
/**
 * Lọc "nhập học": chỉ áp cho chế độ thâm niên (tenure).
 * Nếu camp.enrollFrom = 'YYYY-MM-DD' -> chỉ nhận NV có NGÀY VÀO LÀM >= mốc đó.
 * (Người vào trước mốc không thuộc chương trình — dùng cho "lớp bắt đầu từ ngày X".)
 */
function enrolledIn(camp, profile) {
  if (camp.anchorMode === 'campaign') return true; // campaign mode tính theo lịch, không lọc ngày vào làm
  const ef = String(camp.enrollFrom || '').trim();
  if (!ef) return true; // không đặt mốc -> nhận tất cả (giữ hành vi cũ)
  const from = new Date(ef.length <= 10 ? ef + 'T00:00:00Z' : ef);
  if (isNaN(from.getTime())) return true;
  const start = new Date(profile.start_working_date);
  if (isNaN(start.getTime())) return false; // không rõ ngày vào làm -> loại (chặt)
  // So NGÀY theo giờ VN (khớp với hiển thị "Ngày vào" trên UI). start lưu ở UTC nên +7h rồi mới lấy ngày:
  // vd 2026-07-31T17:00Z = 01/08 giờ VN → phải tính là 01/08, không phải 31/07.
  const startVN = new Date(start.getTime() + VN_OFFSET_MS);
  const startDay = Date.UTC(startVN.getUTCFullYear(), startVN.getUTCMonth(), startVN.getUTCDate());
  const fromDay = Date.UTC(from.getUTCFullYear(), from.getUTCMonth(), from.getUTCDate());
  return startDay >= fromDay;
}
...
function computeSends(rosterActive, campaigns, sentLog = {}, now = new Date(), profilesById = {}) {
  ...
  for (const camp of campaigns) {
    ...
    // Đối tượng: nếu có targetIds -> đúng những người đó (bỏ qua bộ lọc, kể cả NV cũ); ngược lại -> roster (NV mới) + lọc.
    const ids = (camp.targetIds || []).map(Number).filter(Number.isFinite);
    const fixedList = ids.length > 0;
    const population = fixedList ? ids.map((id) => profilesById[id]).filter(Boolean) : rosterActive;

    for (const p of population) {
      // Danh sách ID cố định = đối tượng đã chốt → KHÔNG áp bộ lọc/active. Lộ trình theo bộ lọc thì mới lọc.
      if (!fixedList && !matchesAudience(p, camp.audience)) continue;
      // Mốc nhập học (chế độ thâm niên): chỉ nhận NV vào làm từ enrollFrom trở đi.
      if (!fixedList && !enrolledIn(camp, p)) continue;
      ...
      for (const m of messages) {
        ...
        sends.push({
          ...
          forced: fixedList, // bắn thẳng theo ID, runner không chặn theo trạng thái active
        });
      }
    }
  }
...
module.exports = { workingDays, campaignDay, parseOrg, isActive, matchesAudience, enrolledIn, renderMessage, computeSends, sentKey };
