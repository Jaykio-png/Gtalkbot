...
async function tick() {
  ...
  // 1) CRAWL — giờ chung. Dùng `hhmm >= crawlTime` (so chuỗi HH:MM, đã zero-pad) thay vì khớp
  //    KHÍT phút: nếu tiến trình đang ngủ/chết/đang redeploy đúng phút crawlTime thì lần tick/khởi
  //    động sau (vẫn trong ngày) sẽ CHẠY BÙ, không mất luôn ngày đó. lastRun.crawl chống lặp trong ngày,
  //    và được nạp từ scanStats.date lúc start() nên sống sót qua restart (không crawl lại nếu đã crawl hôm nay).
  if (cfg.crawlEnabled && hhmm >= cfg.crawlTime && lastRun.crawl !== today) {
    lastRun.crawl = today;
    deps.log(`[scheduler] ${hhmm} VN (mốc ${cfg.crawlTime}) → kích hoạt CRAWL${hhmm !== cfg.crawlTime ? ' (chạy bù)' : ''}`);
    Promise.resolve().then(() => deps.runCrawl()).catch((e) => deps.log('[scheduler] crawl lỗi: ' + e.message));
  }

  // 1b) NV MỚI (Data API) — lịch riêng, cũng dùng `>=` để chạy bù nếu server ngủ/redeploy đúng phút đó.
  //     lastRun nạp từ ngày chạy gần nhất lúc start() nên sống sót qua restart.
  if (cfg.newHiresEnabled && hhmm >= cfg.newHiresTime && lastRun.newHires !== today && deps.runNewHires) {
    lastRun.newHires = today;
    deps.log(`[scheduler] ${hhmm} VN (mốc ${cfg.newHiresTime}) → lấy NV mới${hhmm !== cfg.newHiresTime ? ' (chạy bù)' : ''}`);
    Promise.resolve().then(() => deps.runNewHires()).catch((e) => deps.log('[scheduler] NV mới lỗi: ' + e.message));
  }
...
async function start(d) {
  ...
  // Nạp mốc "đã crawl ngày nào" từ Supabase (scanStats.date) để CHẠY BÙ đúng qua restart:
  //  - Nếu ngày đó = hôm nay → coi như đã crawl, không chạy lại.
  //  - Nếu là ngày cũ hơn (vd sự cố 27/7) → lastRun.crawl là ngày cũ ≠ hôm nay → tick sẽ crawl bù ngay.
  try {
    const roster = await store.getRoster();
    const lastDate = roster && roster.scanStats && roster.scanStats.date;
    if (lastDate) lastRun.crawl = lastDate;
  } catch { /* bỏ qua, mặc định sẽ crawl bù */ }
  // Tương tự cho "NV mới": ngày chạy gần nhất = khóa mới nhất trong store, để không chạy lại trong ngày.
  try {
    const nh = await store.getNewHires();
    const dates = Object.keys(nh || {}).sort();
    if (dates.length) lastRun.newHires = dates[dates.length - 1];
  } catch { /* bỏ qua, mặc định sẽ chạy bù */ }
  ...
  deps.log(`[scheduler] đã bật (giờ VN) — crawl=${c.crawlEnabled ? c.crawlTime : 'tắt'}, NV mới=${c.newHiresEnabled ? c.newHiresTime : 'tắt'}, lộ trình tự gửi: ${times.length ? times.join(', ') : 'không có'}`);
}
